...
class Alumno {
    ...
    public function borrarAlumno($id_alumno, $conn) {
        ...
        $query = "DELETE FROM asistencias WHERE id_alumno = :id_alumno";
    ...
    public function obtenerAlumnosPresentes($id_alumno, $conn) {
        $query = "SELECT COUNT(*) FROM alumnos a INNER JOIN asistencias b ON a.id_alumno = b.id_alumno WHERE a.id_alumno = :id_alumno";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':id_alumno', $id_alumno, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
